jQuery(document).ready(function ($) {
    ...
    timelines.each(function () {
      ...
      timelineComponents["eventsWrapper"].on("click", "a", function (event) {
        ...
        autoSlideTimer = setInterval(function () {
          showNewContent(timelineComponents, timelineTotWidth, "next");
        }, 6000); // Inicia um novo temporizador de 2 segundos
  ...
  function updateSlide(timeline) {
    var translateValue = getTranslateValue(timeline),
      wrapperWidth = Number(timeline.style.width.replace("px", "")),
      eventsWrapper = timeline.querySelector(".events-wrapper"),
      events = timeline.querySelectorAll(".timeline-event");

    // Obter a posição do evento selecionado (por exemplo, o evento atual)
    var selectedEvent = timeline.querySelector(".selected");
    var selectedEventOffset = selectedEvent.offsetLeft;

    // Verificar se é o último evento
    var isLastEvent = selectedEvent.nextElementSibling === null;

    // Calcular o deslocamento necessário para centralizar o evento selecionado na tela
    var slideTo;
    if (isLastEvent) {
      var firstEvent = events[0];
      slideTo =
        -(firstEvent.offsetLeft - wrapperWidth / 2) +
        selectedEvent.offsetWidth / 2;
      firstEvent.classList.add("selected");
      selectedEvent.classList.remove("selected");
    } else {
      slideTo =
        -(selectedEventOffset - wrapperWidth / 2) +
        selectedEvent.offsetWidth / 2;
    }

    // Atualizar a posição do cronograma
    translateTimeline(eventsWrapper, slideTo);
  }

  // Exemplo de uso
  var timeline = document.querySelector(".my-horizontal-timeline");
  updateSlide(timeline);
  ...
  function showNewContent(timelineComponents, timelineTotWidth, string) {
    ...
    if (newContent.length > 0) {
      ...
      var firstEvent =
        timelineComponents["eventsWrapper"].find("li:first-child a");
      updateFilling(
        firstEvent,
        timelineComponents["fillingLine"],
        timelineTotWidth
  ...
  function startAutoSlide(timelineComponents, timelineTotWidth) {
    clearInterval(autoSlideTimer); // Cancela o temporizador atual
    autoSlideTimer = setInterval(function () {
      showNewContent(timelineComponents, timelineTotWidth, "next");
  ...
  setInterval(function() {
    var selectedEvent = timelineComponents.eventsWrapper.find('.selected');
    var nextEvent = selectedEvent.next();
  
    // Verifica se o próximo evento é o primeiro evento
    if (nextEvent.length === 0) {
      nextEvent = timelineComponents.eventsWrapper.find('li:first-child a');
    }
